#include "SampleAnalyzer/User/Analyzer/WZPol.h"
using namespace MA5;
using namespace std;
...
// -----------------------------------------------------------------------------
bool WZPol::Initialize(const MA5::Configuration& cfg, const std::map<std::string,std::string>& parameters)
{
  cout << "BEGIN Initialization" << endl;
  
  // outfile_ = TFile::Open("histout.root", "recreate");

  hist_cosThetaW = new TH1F("cos ThetaW", "cos ThetaW", 50, -1, 1);
  hist_cosThetaZ = new TH1F("cos ThetaZ", "cos ThetaZ", 50, -1, 1);
...
// -----------------------------------------------------------------------------
void WZPol::Finalize(const SampleFormat& summary, const std::vector<SampleFormat>& files)
{
  cout << "BEGIN Finalization" << endl;

  // saving histos
  // Write the output of Madanalysis to a file with the name "output_wTzT.root"
  TFile* output = new TFile("output_wTzT.root", "RECREATE");
  // Set the title of the X and Y axes, respectively
  hist_cosThetaW->GetXaxis()->SetTitle("cos#theta^{*}");
  hist_cosThetaW->GetYaxis()->SetTitle("Events");

  hist_cosThetaZ->GetXaxis()->SetTitle("cos#theta^{*}");
  hist_cosThetaZ->GetYaxis()->SetTitle("Events");

  hist_cosThetaW->Write();
  hist_cosThetaZ->Write();
  output->Close();  
...
// -----------------------------------------------------------------------------
bool WZPol::Execute(SampleFormat& sample, const EventFormat& event)
{
  if (event.mc() == 0)
    return false;
  // ***************************************************************************
  ...
  // Initialization of pointers for positive and negative leptons, the z boson, and the w boson

  const MCParticleFormat* z = 0;
  const MCParticleFormat* w = 0;
  const MCParticleFormat* lepton_neu = 0;
  const MCParticleFormat* lepton_muplus = 0;
  const MCParticleFormat* lepton_muminus = 0;
  const MCParticleFormat* lepton_eminus = 0;
  const MCParticleFormat* lepton_eplus = 0;
  ...
  //std::cout << "Number of particles in this event = " << event.mc()->particles().size() << endl;
    ...
    for (unsigned int i=0;i<event.mc()->particles().size();i++){
      ...
      if ( std::abs(prt->pdgid())!=11 && std::abs(prt->pdgid())!=12 && std::abs(prt->pdgid())!=13 ) continue; 
      // PDGID 11 = electron; PDGID 12 = electron neutrino; PDGID 13 = muon

      // Getting the mother of the lepton and checking if it is a Z or W boson
      const MCParticleFormat* mother = prt->mothers()[0];

      if (mother==0) continue;
      if (std::abs(mother->pdgid())!=23 && std::abs(mother->pdgid()) !=24) continue;

      // Looking for a positive lepton in the event
      // -------- electrons / neutrinos from W --------
      if (std::abs(prt->pdgid()) == 12) { // && positive < 1){
          lepton_neu = prt;
          w = mother;
          // positive++;
  ...
  if (!z || !w || !lepton_neu) return true;
  if ((!lepton_muplus && !lepton_muminus)) return true;
  if ((!lepton_eplus && !lepton_eminus)) return true;

  const MCParticleFormat* lepton_mu = lepton_muplus ? lepton_muplus : lepton_muminus;
  const MCParticleFormat* lepton_el = lepton_eplus ? lepton_eplus : lepton_eminus;
  /*
  leppt[0] = lepton_neu;
  leppt[1] = lepton_muon;
  leppt[2] = lepton_eminus;
  leppt[3] = lepton_eplus;
  ...
  */
  ...
  MCParticleFormat lepton_new = *lepton_mu;
  lepton_new.ToRestFrame(z);

  MCParticleFormat lepton2_new = *lepton_el;
  lepton2_new.ToRestFrame(w);

  // Filling histogram
  hist_cosThetaZ->Fill( cos(lepton_new.angle(z)), event.mc()->weight() );
  hist_cosThetaW->Fill( cos(lepton2_new.angle(w)), event.mc()->weight() );
